<?php
    while($row = mysqli_fetch_assoc($query)){
        ...
        if((mysqli_num_rows($query2) == 0)) {
            echo '¡No hay ningún usuario para confirmar operaciones!';
        }
        $row2 = mysqli_fetch_array($query2);
        ($row['status'] == "Offline now") ? $offline = "offline" : $offline = "";
        ($outgoing_id == $row['id']) ? $hid_me = "hide" : $hid_me = "";
        $output = mysqli_query($conexion, "SELECT * FROM chats WHERE (incoming_msg_id = {$_SESSION['user']}) ORDER BY msgdate DESC");
        while($out = mysqli_fetch_array($output)){ 
            ...
            <input type="checkbox" class="userlistcheck" name="userlistchecked" id="'.$usx['user'].'" value="'.$usx['user'].'">            
            </div>';
    }
}
?>
